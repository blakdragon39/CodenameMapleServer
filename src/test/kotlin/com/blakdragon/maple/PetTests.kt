...
class PetTests {
    ...
    fun beforeAll() {
        ...
        user = loginController.login(loginRequest)
    }
    ...
    @Test
    fun createPets() {
        val catResponse = petController.createPet(user.token!!, CreatePetRequest(user.id!!, PetSpecies.Cat))

        assertEquals(catResponse.species, PetSpecies.Cat)
        assertEquals(petController.getAll().size, 1)

        val dragonResponse = petController.createPet(user.token!!, CreatePetRequest(user.id!!, PetSpecies.Dragon))
    ...
    fun createPetWrongUser() {
        try {
            petController.createPet(user.token!!, CreatePetRequest("Wrong ID", PetSpecies.Dog))
    ...
    fun createPetWrongAuth() {
        try {
            petController.createPet("Wrong Token", CreatePetRequest(user.id!!, PetSpecies.Cow))
        } catch (e: ResponseStatusException) {
